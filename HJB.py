...
nn2_ForA = [d, 75, 75, d]
nn3_ForGamma = [d, 50, 50, d**2]
nn3_ForA = [d, 50, 50, d]
nn4_ForGamma = [d, 125, 125, d**2]
nn4_ForA = [d, 125, 125, d]
...
def f_tf(t, X, Y, Z, Gamma):
	return -tf.reduce_sum(tf.square(Z),1,keepdims=True)


def g_tf(X):
	return tf.math.log(0.5*(tf.reduce_sum(tf.square(X),1,keepdims=True)+1))
...
def _batch_norm(x, name):
	with tf.compat.v1.variable_scope(name,reuse=tf.compat.v1.AUTO_REUSE):
		...
		gamma = tf.compat.v1.get_variable('gamma',params_shape,tf.float64,initializer=tf.random_uniform_initializer(
								0.1, 0.5))
...
def _cnn_time_net(x,name,isgamma=False):
	with tf.compat.v1.variable_scope(name):
		input_x = tf.reshape(x,[64,16,16,1])
...
def _nn_layer(input_, output_size, activation_fn=tf.nn.relu, stddev=5.0, name='linear'):
	with tf.compat.v1.variable_scope(name,reuse=tf.compat.v1.AUTO_REUSE):
		...
		w = tf.compat.v1.get_variable('Matrix',[256,output_size],tf.float64,
							tf.random_normal_initializer(
								stddev=stddev/np.sqrt(256+output_size)))
...
with tf.compat.v1.Session(config=config) as sess:
	...
	# if d=256 or 400
# 	learning_rate = tf.compat.v1.train.piecewise_constant(global_step,[999], 
#                                                           [0.01,0.005])
	# if d=100 
	learning_rate = tf.compat.v1.train.exponential_decay(0.01,global_step,decay_steps=1000, 
	...
	try :
		...
		for _ in range(n_maxstep + 1):
			...
			error = np.absolute(y0_value - 4.5901)/4.5901
